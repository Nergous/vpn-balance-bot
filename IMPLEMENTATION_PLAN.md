...
1. Состояние незавершённой payment-формы хранится в SQLite в одной записи на администратора.
2. До финального подтверждения БД не меняется.
3. Отмена удаляет wizard state.
4. Рестарт приложения сохраняет незавершённый wizard; запись удаляется только после успешного ledger write или явной отмены.
5. Перед payment показываются user, amount, date, comment и итоговое действие.
6. Повторное command/callback-подтверждение не должно создавать вторую операцию; state-changing update ID обрабатывается транзакционно.
...
- отмена wizard;
- restart с восстановлением незавершённого wizard;
...
Этап 14 завершён: backup создаётся штатной CLI-командой и проверяется автоматически. Этап 15 остаётся открытым до добавления deployment packaging, полного runbook и ручной приёмки с отдельными test credentials.
